...
var data= [
    ...
    function child_clicked(e){
       ...
       var create_div =document.createElement("div")
        ...
        `
        document.querySelector('.output').append(create_div)
    }

next.addEventListener("click",()=>{
    var userdata= data.length;
    var iterate_value=Number(userdata/10)*10;
    for(var i=1; i<=10; i++){
    var btn_clicked = document.querySelector(`.btn-${i}`)
    var get_att = btn_clicked.getAttribute('for')
    if( iterate_value>=get_att){
    var add_att = Number(get_att) + 10
    var set_att = btn_clicked.setAttribute('for',add_att)
    btn_clicked.innerText = add_att
    }
}
})
